"""
Export all 10 SyncMaster Carousel 02 artboards to a PPTX.
Each slide = pixel-perfect screenshot of the rendered design.
"""

import asyncio, os, io, threading, http.server, functools
...
HTML_NAME   = "SyncMaster Carousel 02.html"
PORT        = 18766   # different port so Carousel 01 export can run simultaneously
...
async def screenshot_slides():
    ...
    async with async_playwright() as p:
        ...
        # Wait for all 10 artboards + fonts to settle
        await page.wait_for_function(
            "document.querySelectorAll('[data-dc-slot]').length >= 10",
            timeout=30000
        )
...
def build_pptx(pngs):
    ...
    out = os.path.join(PROJECT_DIR, "SyncMaster_Carousel_02.pptx")
    prs.save(out)
    return out
...
async def main():
    ...
    print(f"\nDone -> SyncMaster_Carousel_02.pptx  ({kb} KB)")
